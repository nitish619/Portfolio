...
function Skills() {
  const skills = [
    ...
    {
      name: "Express.js",
      icon: <SiExpress className="text-yellow-300 text-4xl" />,
    },
    ...
    {
      name: "MERN Stack",
      icon: <SiMongodb className="text-green-900 text-4xl" />,
    },
  ...
  return (
    <section
      id="skills"
      className="container mx-auto w-full py-16 h-auto flex flex-col items-center"
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-8 p-6 justify-center">
        {skills.map((skill, index) => (
          <div
            key={index}
            className="flex flex-col items-center p-4 bg-slate-800 rounded-xl shadow-md transition-transform hover:scale-105 hover:bg-slate-700 hover:shadow-lg hover:shadow-slate-400"
          >
            {skill.icon}
